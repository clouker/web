...
lyGrid = function (options) {
    ...
    var grid = document.getElementById(conf.pagId);
    var l_tree = conf.treeGrid;
    var col = [];
    for (var i = 0; i < conf.l_column.length; i++)
        col.push(l_col)
    for (var i = 0; i < col.length; i++) {
        for (var p in col[i]) if (col[i].hasOwnProperty(p) && (!conf.l_column[i].hasOwnProperty(p))) conf.l_column[i][p] = col[i][p]
    }
    ...
    var pageBind = function (e) {
        ...
        if (b) {
            ...
            if (String(conf.local) == "true") {
                ...
                conf = $.extend(conf, {pageSize: parseInt(pageSize, 10)});
                cBodytb(returnData);
                if (String(conf.usePage) == "true") {
                    fenyeDiv(returnData);
                    lyickeck()
    ...
    var sortBind = function () {
        ...
        conf.data = $.extend(conf.data, {sidx: t[0], sord: sc});
    ...
    var setOptions = function (options) {
        var data;
        if (options.data) {
            data = $.extend(conf.data, options.data);
            options.data = data
        }
        if (options.pageSize) {
    ...
    var jsonRequest = function () {
        ...
        conf = $.extend(conf, {pagecode: json.pagecode, pageSize: json.pageSize});
        return json
    };
    init();
    return {
